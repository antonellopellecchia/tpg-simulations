...
int main(int argc, char * argv[]) {
	...
	//double electricFields[fieldNumber] = {.1, 1., 10., 100., 1000.};
	TGraphErrors *velocityGraph = new TGraphErrors(fieldNumber);
	const unsigned int nEvents = atoi(argv[1]);
	for (int fieldIndex = 0; fieldIndex < fieldNumber; fieldIndex++) {
		double electricField = 100. + fieldIndex*100.;
	...
	sprintf(pathstr, "/lustrehome/antonellopellecchia/simulations/drift_velocity/results/velocity_%devents_%s.root", nEvents, datetime->AsString());
	c->SaveAs(pathstr);
	sprintf(pathstr, "/lustrehome/antonellopellecchia/simulations/drift_velocity/results/velocity_%devents_%s.pdf", nEvents, datetime->AsString());
	c->SaveAs(pathstr);
